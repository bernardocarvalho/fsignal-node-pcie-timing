...
//  COMMAND_REG    cReg;
  ...
  PDEBUG("_pcie_read  NOT implemented \n");
#ifdef _DMA
...
int configurePCI(PCIE_DEV *pcieDev) {
  ...
  for (i = 0; i < NUM_BARS; i++) {
    ...
    // virtual addr
    pcieDev->memIO[i].vaddr = ioremap_cache(pcieDev->memIO[i].start,
					      pcieDev->memIO[i].len);
    ...
    if (!pcieDev->memIO[i].vaddr) {
      printk(KERN_ERR "_pcie: error in ioremap_cache [%d]. Aborting.\n", _ret);
      return -ENOMEM;
    }
  }
