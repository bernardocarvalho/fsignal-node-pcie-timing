...
struct DataProducerInit{
    PcieAdcATCANode *node;
    FSHardware      *hardware;
};

const string PcieAdcATCANode::DEV_FILE_LOCATIONS_ID = "DevFileLocations";
const string PcieAdcATCANode::FS_BUFFER_SEND_SIZE   = "BufferSendSize";

PcieAdcATCANode::PcieAdcATCANode(const char* propsLoc, wstring mainEventID, const char *logFileLoc) : FSNode(propsLoc, logFileLoc), mainEventID(mainEventID){
    ...
    for(unsigned int i=0; i<hwXmls.size(); i++) {
        ...
        for(unsigned int j=0; j<parXmls.size(); j++) {
            PcieAdcATCAParameter *parameter = new PcieAdcATCAParameter(parXmls[j], j+1);                        
...
void PcieAdcATCANode::newDataAvailable(const CORBA::WChar* nodeUniqueID, const CORBA::WChar* hardwareUniqueID, const CORBA::WChar* parameterUniqueID, const fscns::hardware::EventList& eList, const fscns::util::TimeStamp& tstart, const fscns::util::TimeStamp& tend) {    
}

void PcieAdcATCANode::abort(){
    logStream.str("");
...
void PcieAdcATCANode::setEventTable(const org::cfn::scad::core::corba::hardware::EventList& evtList) {
...
void *PcieAdcATCANode::dataProducer(void* args) {
    ...
    while(true){
	  ...
	  if(hwParameters.size() > 0){
	    ...
	    if(parameter != NULL){
	      ...
	      logStream.str("");
	      logStream << "Delay for " << node->devIds[i] << ':' << delay;
	      LOG4CXX_INFO(Utils::getLogger(), logStream.str());
	      ret = ioctl(fd[i], PCIE_ADC_IOCS_TRIG_DLY, &delay);
	      if(ret < 0){
		logStream.str("");
                logStream << "While setting trig delay acquisition, error ioctling the device " << node->devIds[i];
                LOG4CXX_INFO(Utils::getLogger(), logStream.str());
	      }
	    }
	  }
	  ...
	  if(ret < 0){
                logStream.str("");
                logStream << "While enabling the interrupts, error ioctling the device " << node->devIds[i];
                LOG4CXX_INFO(Utils::getLogger(), logStream.str());
	  }
	  ret = ioctl(fd[i], PCIE_ADC_IOCT_ACQ_ENABLE);
	  if(ret < 0){
	    logStream.str("");
	    logStream << "While enabling acquisition, error ioctling the device " << node->devIds[i];
	    LOG4CXX_INFO(Utils::getLogger(), logStream.str());
	  }
	  ...
	  if (mster & 0x00400000)
        ...
        if (masterBoardId != -1) { // Master Board Present
	  ...
	  std::vector<FSParameter*> hwParameters = hardware->getAllParameters();
          if(hwParameters.size() > 0){
            ...
            if(parameter != NULL){
              if(parameter->getTriggerType() == PcieAdcATCAParameter::TRG_TYPE_SOFTWARE){
                logStream.str("");
                logStream << "Sending software trigger to Master device "
                          << node->devIds[masterBoardId];
                LOG4CXX_INFO(Utils::getLogger(), logStream.str());

                if(ret < 0){
                  logStream.str("");
                  logStream << "While sending the software trigger, error ioctling the device "
                            << node->devIds[masterBoardId];
                  LOG4CXX_INFO(Utils::getLogger(), logStream.str());
                }
              }
        ...
        else {           // Master NOT present. Send Soft trigger to all slaves
          for(int i=0; i<node->nhardware; i++){
            hardware  = dpi[i].hardware;
            if(!hardware->isConnected()){
              continue;
            ...
            if(hwParameters.size() > 0){
              ...
              if(parameter != NULL){
                if(parameter->getTriggerType() == PcieAdcATCAParameter::TRG_TYPE_SOFTWARE){
                  ...
                  logStream.str("");
                  logStream << "Sending software trigger to Slave device " << node->devIds[\
											   i];
                  LOG4CXX_INFO(Utils::getLogger(), logStream.str());
                  ret = ioctl(fd[i], PCIE_ADC_IOCT_SOFT_TRIG);
                  if(ret < 0){
                    logStream.str("");
                    logStream << "While sending the software trigger, error ioctling the de\
vice "
                              << node->devIds[i];
                    LOG4CXX_INFO(Utils::getLogger(), logStream.str());
                  }
	...
	*/             
        ...
        while(nAcqCompleted != 0 && !node->aborted){
            ...
            for(int i=0; i<node->nhardware; i++){
                ...
                if(ret < 0){
                    logStream.str("");
                    logStream << "While waiting for acquisition acquisition complete, error ioctling the device " << node->devIds[i];
                    LOG4CXX_INFO(Utils::getLogger(), logStream.str());
                    acqComplete = true;
                    continue;
                }
		...
		*/
        ...
        for(int i=0; i<node->nhardware; i++){
            ...
            for(unsigned int j=0; j < hwParameters.size(); j++){
                ...
                nBytes = (int)(parameter->getAcqTimeMillis() * 1e-3 * parameter->getAcquisitionFrequency() * sizeof(int));
		...
		/*                //find next power of 2
                ...
                */
                ...
                while(nBytes > 0){
                    ...
                    offset = j * parameter->getChannelOffset() + counter * bufferSize;                
                    counter++;
                    ret = ioctl(fd[i], PCIE_ADC_IOCS_RDOFF, &offset);                
                    if(ret < 0){
                        logStream.str("");
                        logStream << "While setting the offset to: " <<  offset << ", error ioctling the device " << node->devIds[i];
                        LOG4CXX_INFO(Utils::getLogger(), logStream.str());
                        continue;
                    }            
                  ...
                  /*  ret = ioctl(fd[i], PCIE_ADC_IOCT_DMA_ENABLE);                
                    ...
                    for(unsigned int s=0; s<(readBytes / sizeof(int)); s++){
                        *tempSwap = (copyBuffer[4 * s] << 24) | (copyBuffer[4 * s + 1] << 16) | (copyBuffer[4 * s + 2] << 8) | (copyBuffer[4 * s + 3]);
                        tempSwap++;
                    }
                    nBytes -= readBytes;
                    if(readBytes > 0){
                    ...
                    if((bufferToSendSize > node->bufferSendSize)){
                        ...
                        int npoints=bufferToSendSize / sizeof(int);
                        tmp16=(short*)(buffer);
                        copyBuffer16=new short[npoints];
                        for(int j=0;j<npoints;++j)
                        {
                            //tmp16+=2;
                            copyBuffer16[j]=*tmp16;
                            tmp16+=2;
                        }
	...
	for(int i=0; i<node->nhardware; i++){
	  ...
	  if(ret < 0){
	    logStream.str("");
	    logStream << "While disabling acquisition, error ioctling the device " << node->devIds[i];
	    LOG4CXX_INFO(Utils::getLogger(), logStream.str());
	    acqComplete = true;
	    continue;
	  }
	  ...
	  if(ret < 0){
	    logStream.str("");
	    logStream << "While disabling interrupts, error ioctling the device " << node->devIds[i];
	    LOG4CXX_INFO(Utils::getLogger(), logStream.str());
	    acqComplete = true;
	    continue;
	  }
