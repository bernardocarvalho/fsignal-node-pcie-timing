...
vice " << node->devIds[i];
	...
	*/
        ...
        while (nAcqCompleted > 0 && !node->aborted)
        {
            ...
            {
                hardware = dpi[i].hardware;
                if (!hardware->isConnected () || hardware->getStatus()==hardware::STOPPED)
                {
                    nAcqCompleted--;
                    continue;
                }
		...
		*/
        ...
        if (!node->aborted)
        {
            ...
            int numberOfOnlineHardware = node->nhardware;
            int *tempSwap;
            for (int i = 0; i < node->nhardware; i++)
            {
                numberOfConnectedChannels = 0;
                hardware = dpi[i].hardware;
                if (!hardware->isConnected () || hardware->getStatus()==hardware::ERROR_STATE || hardware->getStatus()==hardware::STOPPED)
                {
                    numberOfOnlineHardware--;
                    if(numberOfOnlineHardware<=0)
                    {
                        LOG4CXX_INFO (Utils::getLogger (),"Acquisition cancelled for hardware " +
