...
int main(int argc, char **argv)
{
    ...
    uval =0;
    rc = ioctl(fd, KX1_PCIE_IOCG_MOD_STATUS, &uval);
    printf("rc:%d, MOD_STATUS 0x%08x\n", rc, uval);
    uval =0;
    ...
    //tRegs.evnt_regs.config=   0x3000000A;// 0xC0000000; //0x80000000 LED ON 2/A:1 C:2
    tRegs.evnt_regs.config=   0x90000000;// 0xC0000000;
    for(i=0; i < NUM_TIMERS; i++){
        tRegs.channel=i;
        tRegs.evnt_regs.delay= (i+1) * 10000000;
        rc = ioctl(fd, KX1_PCIE_IOCS_TIMER_REGS, &tRegs);
    }
