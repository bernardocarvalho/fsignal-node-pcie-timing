...
namespace fsignal {
    class TestNode : public FSNode {
    public:
        TestNode(const char* propsLoc, std::wstring mainEventID, const char *logFileLoc=NULL);
        ~TestNode(){
            evtSem.Close();
        }
        ...
        bool getDataCluster(TestParameter &par, TimeStampWrapper &tstart, TimeStampWrapper &tend, std::vector<EventWrapper> &evtList, float *buffer);
