 ...
 ***************************************************************************/
#ifndef FSIGNAL_CENTRAL_TEST_NODE_H
#define FSIGNAL_CENTRAL_TEST_NODE_H
#include "../FSNode.h"
...
namespace fsignal {
    class CentralTestNode : public FSNode {
    public:
        CentralTestNode(const char* propsLoc, std::wstring mainEventID, const char *logFileLoc=NULL);
        ~CentralTestNode(){
            evtSem.Close();
        }
        ...
        static void* operManager(void* args);
        bool getDataCluster(CentralTestParameter &par, TimeStampWrapper &tstart, TimeStampWrapper &tend, std::vector<EventWrapper> &evtList, float *buffer);
    private:
        EventSem evtSem;
